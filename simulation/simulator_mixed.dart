...
void main(List<String> args) async {
  print('${DateTime.now()} running mixed simulator...');
  ...
  if (args[1] == 'changing') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_changing_noFear_noChat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useSChMUSR: false,
        folder:
            '../results/results_withCats_SChMUSR_changing_noFear_noChat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'noFearNoChatNotCAB') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_notCAB_mixed_noFear_noChat_${args[0]}',
        numGens);
    ...
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_mixed_fear_noChat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useSChMUSR: true,
        folder:
            '../results/results_withCats_SChMUSR_mixed_fear_noChat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'noFearChat') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_mixed_noFear_chat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useFear: false,
        folder:
            '../results/results_withCats_SChMUSR_mixed_noFear_chat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'fearChat') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_mixed_fear_chat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useSChMUSR: true,
        folder:
            '../results/results_withCats_SChMUSR_mixed_fear_chat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  }
...
Future<void> evolve({
  ...
  for (var gen = startIndex; gen < numGens; gen++) {
    ...
    for (var game = 0; game < gamesPerGen; game++) {
      ...
      if (changing) {
        final homogeneous = Random().nextBool();

        if (homogeneous) {
          final idx = Random().nextInt(popSize);
      ...
      final theGenes = agents.map((a) => a.gene).toList();
