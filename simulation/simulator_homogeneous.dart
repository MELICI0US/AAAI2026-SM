...
void main(List<String> args) async {
  print('${DateTime.now()} running homogeneous simulator...');
  ...
  const numCats = 2;
  final extraAgents = <CatAgent Function(AutoDisposeProviderRef ref)>[
    for (int i = 0; i < numCats; i++) CatAgent.new
  ];

  if (args[1] == 'noFearNoChat') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_homogeneous_noFear_noChat_${args[0]}',
        numGens);
    ...
    await evolve(
        ...
        useSChMUSR: false,
        folder:
            '../results/results_withCats_SChMUSR_homogeneous_noFear_noChat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'noFearNoChatNotCAB') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_notCAB_homogeneous_noFear_noChat_${args[0]}',
        numGens);
    ...
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_homogeneous_fear_noChat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useSChMUSR: true,
        folder:
            '../results/results_withCats_SChMUSR_homogeneous_fear_noChat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'noFearChat') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_homogeneous_noFear_chat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useFear: false,
        folder:
            '../results/results_withCats_SChMUSR_homogeneous_noFear_chat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  } else if (args[1] == 'fearChat') {
    final lastSuccessfulGen = lastSucccessfulGeneration(
        'results_withCats_SChMUSR_homogeneous_fear_chat_${args[0]}', numGens);
    ...
    await evolve(
        ...
        useSChMUSR: true,
        folder:
            '../results/results_withCats_SChMUSR_homogeneous_fear_chat_${args[0]}',
        timeout: timeout,
        extraAgents: extraAgents);
  }
...
Future<void> evolve({
  ...
  if (startIndex == 0) {
    ...
    theGenePools =
        evolvePopulationPairs(theGenePoolsOld, popSize, numGeneCopies);
  }
...
List<GeneMetrics> evolvePopulationPairs(
    List<GeneMetrics> theGenePoolsOld, int popSize, int numGeneCopies) {
  ...
  for (var i = 0; i < popSize; i++) {
    ...
    for (var g = 0; g < theGenePoolsOld[0].numGenes; g++) {
      const minKeepIdx = 12;
      if (g == minKeepIdx) {
        //FIXME: this is hardcoded to ignore minKeep
        geneStr += '0';
...
Future<List<PopularityMetrics>> playGame(
  ...
  watchGame.listen((event) async {
    ...
    if (event.roundInfo.status.isGameOver) {
      ...
      final agentNameToGene = <(String, String)>[
        for (final sub in subs) (sub.read().myPlayerName, sub.read().genes!)
      ];

      final extraAgentNames = <String>[
